...
local function open_panel(left, right, row, col, width, right_col)
    ...
    local height = #left
    local blank = {}
    for i = 1, height do blank[i] = "" end
    vim.api.nvim_buf_set_lines(buf, 0, -1, false, blank)
    for i = 1, height do
        vim.api.nvim_buf_set_extmark(buf, ns, i - 1, 0, { virt_text = left[i], virt_text_pos = "overlay" })
        if right[i] then
            vim.api.nvim_buf_set_extmark(buf, ns, i - 1, 0, { virt_text = right[i], virt_text_win_col = right_col })
        end
    end
...
function M.show_stats()
    ...
    local left = {
        ...
        { { "󰚥 ", "Statement" }, { string.format("Plugins: %d/%d", s.loaded, s.count), "Normal" } },
    }
    local right = {
        nil,
